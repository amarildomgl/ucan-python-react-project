...
const Profile = () => {
  ...
  const allSubscriptions = Array.isArray(profile?.subscricoes) ? profile.subscricoes : [];
  const activeSubscriptions = allSubscriptions.filter((sub) => sub.ativa);
  const inactiveSubscriptions = allSubscriptions.filter((sub) => !sub.ativa);
  console.log('DEBUG profile:', profile);
  console.log('DEBUG subscricoes:', allSubscriptions);
      ...
      <div className="container mx-auto px-4">
        ...
        <Tabs defaultValue="profile" className="w-full">
          ...
          <TabsContent value="subscription">
            <Card>
              <CardHeader>
                <CardTitle>Minhas Subscrições</CardTitle>
                <CardDescription>
                  Veja todas as suas subscrições, ativas e antigas.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isProfileLoading ? (
                  ...
                  <div className="space-y-6">
                    {allSubscriptions.map((sub) => (
                      <div key={sub.id} className={`bg-gray-50 p-6 rounded-lg border ${sub.ativa ? 'border-green-400' : 'border-gray-200'}`}>
                        <div className="flex items-center mb-4">
                          <div className={`mr-4 p-2 rounded-full ${sub.ativa ? 'bg-green-100' : 'bg-gray-200'}`}> 
                            {sub.ativa ? <CheckCircle className="h-6 w-6 text-green-600" /> : <X className="h-6 w-6 text-gray-400" />}
                          </div>
                          <div>
                            <h3 className="font-bold text-lg">Plano {sub.plano.nome}</h3>
                            <p className="text-sm text-gray-500">
                              Status: <span className={sub.ativa ? 'text-green-600 font-medium' : 'text-gray-500'}>{sub.ativa ? 'Ativa' : 'Inativa'}</span>
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="mb-4">
                          <p className="text-sm text-gray-500 mb-1">Brinquedos atuais:</p>
                          {sub.brinquedos && sub.brinquedos.length > 0 ? (
                            <ul className="list-disc pl-5 text-sm text-gray-700">
                              {sub.brinquedos.map((b) => (
                                <li key={b.id}>
                                  {b.nome} <span className="text-xs text-gray-400">(Empréstimo: {b.data_emprestimo}{b.data_devolucao ? `, Devolução: ${b.data_devolucao}` : ''})</span>
                                </li>
                              ))}
                            </ul>
                          ) : (
                            <span className="text-xs text-gray-400">Nenhum brinquedo em posse.</span>
                          )}
                        </div>
                        {sub.ativa && (
                          <Dialog>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-xl font-medium mb-2">Nenhuma subscrição encontrada</h3>
